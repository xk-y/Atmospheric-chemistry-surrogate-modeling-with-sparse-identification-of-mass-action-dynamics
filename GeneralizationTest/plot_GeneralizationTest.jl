...
JLD2.@load "../../train/$(n_latent_species)/c_test_utils.jld" ref_data_max ref_data_min dc_std
JLD2.@load "../emit_vary/testing_set_emit_$(@sprintf("%.2f", 2.00)).jld" ref_data_test ref_emit_test ref_params_test specname
pwd()
model_params_test = ref_params_tes
...
JLD2.@load "../../train/$(n_latent_species)/model/simady/stage_5_n_latent_species_$(n_latent_species).jld"  ps_cpu
...
function run_c_test(ps, dataset, dc_std) 
    ...
    for i_case in 1:nruns
        ...
        sol_sindy_ude = cpu(Array(solve(prob2, Rosenbrock23(), saveat=60)))
    ...
    function plot_case(ref, pred, pred_nobuff, time, ind, err_, scenario)
        ...
        for i in ind
            ...
            ptemp = plot(time./1440, ref[:,i];
                         ...
                         left_margin = (scenario == "Best" ? 12Plots.mm : 4Plots.mm ),
